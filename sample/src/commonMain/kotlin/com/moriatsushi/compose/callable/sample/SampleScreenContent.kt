...
internal fun SampleScreenContent(
    onCall: (target: CallableTarget) -> Unit,
    result: String?,
    modifier: Modifier = Modifier,
) {
    var selectedTarget by remember { mutableStateOf(CallableTarget.YES_NO_DIALOG) }
    ...
    Column(
        ...
        Row {
            TargetSelector(
                ...
                onSelectedTargetChange = { selectedTarget = it },
            )
            Button(onClick = { onCall(selectedTarget) }) {
                Text("Call")
            }
        }
        Text(
            modifier = Modifier.padding(vertical = 8.dp),
            text = if (result != null) "Result: $result" else "",
            fontSize = 14.sp,
        )
    }
}
